...
// Temps par lot
$requete = "SELECT SUM(i.heure) as heure, SUM(i.minute) as minute, l.libelle FROM `Imputation` i LEFT JOIN User_Story u ON i.id_us = u.id LEFT JOIN Lot l ON u.id_lot = l.id GROUP BY l.libelle";
...
// Coût par lot
$requete = "SELECT l.libelle, SUM(i.heure* 60 + i.minute * (m.salaire/420)) as cout FROM Imputation i LEFT JOIN Utilisateur u ON i.id_utilisateur = u.id LEFT JOIN Metier m ON u.id_metier = m.id LEFT JOIN User_Story us ON i.id_us = us.id LEFT JOIN Lot l ON us.id_lot = l.id GROUP BY l.libelle";
...
$requete = "SELECT SUM(i.heure) as heure, SUM(i.minute) as minute, t.libelle FROM `Imputation` i LEFT JOIN Tache_Type t ON i.id_tache = t.id GROUP BY t.libelle";
...
$requete = "SELECT t.libelle, SUM(i.heure* 60 + i.minute * (m.salaire/420)) as cout FROM Imputation i LEFT JOIN Utilisateur u ON i.id_utilisateur = u.id LEFT JOIN Metier m ON u.id_metier = m.id LEFT JOIN User_Story us ON i.id_us = us.id LEFT JOIN Tache_Type t ON i.id_tache = t.id GROUP BY t.libelle";
